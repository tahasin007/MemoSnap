package com.android.memosnap.core.theme

import androidx.compose.ui.graphics.Color

val PrimaryLight = Color(0xFF0097A7)
val OnPrimaryLight = Color(0xFFFFFFFF)
val SecondaryLight = Color(0xFF78909C)
val OnSecondaryLight = Color(0xFF5E5E5E)
val SurfaceLight = Color(0xFFFFFFFF)
val OnSurfaceLight = Color(0xFF000000)

val PrimaryDark = Color(0xFF004941)
val OnPrimaryDark = Color(0xFFFFFFFF)
val SecondaryDark = Color(0xFF455A64)
val OnSecondaryDark = Color(0xFFA4A4A5)
val SurfaceDark = Color(0xFF000000)
val OnSurfaceDark = Color(0xFFFFFFFF)

val SoftPink = Color(0xFFF48FB1)
val CrimsonRed = Color(0xFFDC143C)
val Scarlet = Color(0xFFFF2400)
val DodgerBlue = Color(0xFF1E90FF)
val DeepBlue = Color(0xFF1E3A5F)
val GoldenYellow = Color(0xFFF9A825)
val ForestGreen = Color(0xFF388E3C)
val BurntOrange = Color(0xFFEF6C00)
val Lavender = Color(0xFFE1BEE7)
val Teal = Color(0xFF4DB6AC)

val RedColor = Color(0xFFFF0000)
val BlueColor = Color(0xFF0288D1)
val GreenColor = Color(0xFF00796B)
val GrayColor = Color(0xFF808080)